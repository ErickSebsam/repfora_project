  ...
  try {
    ...
    newSchedule.events.forEach((event) => {
      newListEvents.push({
        ...
        start: event ? (typeof event === 'string' ? event : event.toISOString().split("T")[0]) : "",
        tstart: schedule.tstart ? formatDate12Hours(schedule.tstart) : "",
        tend: schedule.tend ? formatDate12Hours(schedule.tend) : "",
        type: 0,
        autogenerated: true,
      });
      index++;
